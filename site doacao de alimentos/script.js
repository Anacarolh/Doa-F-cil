document.addEventListener('DOMContentLoaded', () => {
    ...
    let charts = {};
    let currentRole = null; // 'admin' or 'donor'
    ...
    // --- CUSTOM FOOD ITEM FIELDS ---
    const foodItemSelect = document.getElementById('food-item');
    const foodItemCustom = document.getElementById('food-item-custom');

    foodItemSelect.addEventListener('change', () => {
        if (foodItemSelect.value === 'Outro') {
            foodItemCustom.style.display = 'block';
            foodItemCustom.required = true;
        } else {
            foodItemCustom.style.display = 'none';
            foodItemCustom.required = false;
        }
    });

    const donorFoodItemSelect = document.getElementById('donor-food-item');
    const donorFoodItemCustom = document.getElementById('donor-food-item-custom');

    donorFoodItemSelect.addEventListener('change', () => {
        if (donorFoodItemSelect.value === 'Outro') {
            donorFoodItemCustom.style.display = 'block';
            donorFoodItemCustom.required = true;
        } else {
            donorFoodItemCustom.style.display = 'none';
            donorFoodItemCustom.required = false;
        }
    });
    ...
    donationForm.addEventListener('submit', (e) => { 
        e.preventDefault();
        const selectedItem = foodItemSelect.value === 'Outro' ? foodItemCustom.value.trim() : foodItemSelect.value;
    ...
    donorPledgeForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const selectedItem = donorFoodItemSelect.value === 'Outro' ? donorFoodItemCustom.value.trim() : donorFoodItemSelect.value;
    ...
    const closeEditModal = () => editDonorModal.classList.add('hidden');
    ...
    const deleteDonor = (donorId) => {
        if (confirm('Tem certeza que deseja excluir este doador? Todas as suas doações também serão removidas.')) {
    ...
    // --- DONOR FUNCTIONS ---
    const renderDonorLogin = () => renderDonors(true);
    ...
    // --- EVENT LISTENERS ---
    goToAdminBtn.addEventListener('click', () => adminLoginModal.classList.remove('hidden'));
    goToDonorBtn.addEventListener('click', () => setupUIForRole('donor'));
    logoutBtn.addEventListener('click', logout);

    donorForm.addEventListener('submit', (e) => {
        e.preventDefault();
        addDonor(document.getElementById('donor-name').value, document.getElementById('donor-contact').value);
        donorForm.reset();
    });
    ...
    document.body.addEventListener('click', (e) => {
        ...
        if (button.closest('#pending-donation-list')) {
            if (button.classList.contains('confirm-btn')) confirmDonation(button.dataset.id);
            return;
        }
    });

    donorLoginBtn.addEventListener('click', () => {
        if(donorLoginSelect.value) loginAsDonor(donorLoginSelect.value);
        else alert('Por favor, selecione um doador.');
    });
